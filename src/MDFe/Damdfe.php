...
class Damdfe extends Common
{
        ...
        $mododebug = 2)
    {
        ...
        $this->obs = $this->infMDFe->getElementsByTagName("infCpl")->item(0)->nodeValue;
    }
    ...
    private function headerMDFeRetrato($x, $y, $pag)
    {
        ...
        if (is_file($this->logomarca)) {
            $logoInfo = getimagesize($this->logomarca);
            //largura da imagem em mm
            $logoWmm = ($logoInfo[0] / 72) * 50;
            //altura da imagem em mm
            $logoHmm = ($logoInfo[1] / 72) * 50;
            if ($this->logoAlign == 'L') {
                $nImgW = 16;
                $nImgH = 16;
                $xImg = $x + 1;
    ...
    private function footerMDFe($x, $y)
    {
        ...
        $y += 5;
        $x += 2;
        $this->pTextBox($x, $y, $x2, 8, $this->obs, $aFont, 'T', 'L', 0, '', false);
        $y = $this->hPrint - 4;
